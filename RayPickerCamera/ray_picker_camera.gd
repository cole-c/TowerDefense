...
func _process(delta: float) -> void:
	...
	if(ray_cast_3d.is_colliding()):
		if(bank.gold >= turret_cost):
			...
			if (collider is GridMap && Input.is_action_pressed("click_L")):
				var collision_point = ray_cast_3d.get_collision_point()
				var cell = gridmap.local_to_map(collision_point)
				if(gridmap.get_cell_item(cell) == 0): #0 is unoccupied turret square
					gridmap.set_cell_item(cell, 1)
					var tile_center = gridmap.map_to_local(cell)
					turret_manager.build_turret(tile_center)
